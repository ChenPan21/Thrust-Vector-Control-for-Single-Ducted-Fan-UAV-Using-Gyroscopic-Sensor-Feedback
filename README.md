...
![项目成品图](img/drone_final_assembly.jpg)

这是一个关于使用陀螺仪传感器反馈，为单函道风扇无人机（UAV）实现推力矢量控制（Thrust Vector Control, TVC）的研发项目。项目旨在设计、构建并稳定一台仅依赖单个主动力单元的垂直起降（VTOL）无人机。
...
![TVC 原理](https://i.imgur.com/r62n2zD.gif)  当水瓶开始向左倾斜时，您的手会下意识地向左移动，产生一个反作用力，将水瓶推回中心。当它向右倾斜时，您的手又会向右移动来纠正它。

这台无人机的**推力矢量控制**原理与此非常相似。它底部的四个舵面就像您的手，强大的函道风扇产生的气流就像那瓶水。当无人机的陀螺仪传感器（相当于您的眼睛和平衡感）检测到机身发生倾斜时，控制器会立刻命令相应的舵面偏转，改变高速气流的方向。这样，气流就会产生一个反作用力，将机身“推”回平衡状态。
...
* **结构简化:** 仅需1个函道风扇+4个舵机即可实现全姿态控制，简化了机械结构，减轻了重量和体积。
...
![最终版分层结构图](img/mechanical_v3_layers.png)
...
![软件系统架构图](img/software_architecture_rtos.png)
...
![PID闭环控制流程图](img/pid_workflow.png)
...
![地面推力测试平台](img/thrust_test_setup.jpg)
![推力-时间曲线图](img/thrust_test_graph.png)
...
![系留飞行测试效果图](img/tethered_flight_test.jpg)
